...
public class MyDBHelperTotal extends SQLiteOpenHelper {

    public static final String DATABASE_NAME = "Golf_Total_test.db";
    ...
    public MyDBHelperTotal(@Nullable Context context) {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + TABLE_NAME2 + " (ID INTEGER PRIMARY KEY AUTOINCREMENT, " +
                "CC STRING, DATE STRING, START_TIME STRING, " +
                "PLAYER STRING, HOLE1 STRING, HOLE2 STRING, " +
    ...
    public boolean saveToDB(String CC, String date, String startTIme, String player,
        ...
        long error = db.insert(TABLE_NAME2, null, contentValues);
        if (error == -1)
            return false;
        else
            return true;
    ...
    public Integer deleteData(String cc) {
        SQLiteDatabase db = this.getWritableDatabase();
        return db.delete(TABLE_NAME2, "CC = ?", new String[] {cc});
    }
